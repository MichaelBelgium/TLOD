...
- It doesn't load/save trees dynamicly
- It doesn't load/save cars dynamicly
- It doesn't load players inventory

Y_INI. Makes. Me. Mad :) Compiling takes 12.2 seconds btw, while the SQL takes just 2.7 seconds.
Current problem is just: No idea how to load a whole file into an enum or how to save trees/cars exactly.
*/
...
forward GetUser_Inventory(playerid, name[], value[]);
forward GetUser_Data(playerid, name[], value[]);
forward CheckUser_Password(playerid, name[], value[]);
...
public OnPlayerConnect(playerid)
{
	...
	{
		INI_ParseFile(UserFile(playerid), "CheckUser_%s", .bExtra = true, .extra = playerid);
...
public OnPlayerDisconnect(playerid, reason)
{
	if(Player[playerid][Logged])
	{
		...
		new INI:ufile = INI_Open(UserFile(playerid));
		INI_SetTag(ufile,"Password");
		INI_WriteInt(ufile,"Password",Player[playerid][Password]);

		INI_SetTag(ufile,"Data");
		INI_WriteFloat(ufile,"Hunger",Player[playerid][Hunger]);
		INI_WriteFloat(ufile,"Thirst",Player[playerid][Thirst]);
		INI_WriteFloat(ufile,"Sleep",Player[playerid][Sleep]);
		INI_WriteFloat(ufile,"Exp",Player[playerid][Experience]);
		INI_WriteInt(ufile,"Level",Player[playerid][Level]);
		INI_WriteFloat(ufile,"Pos_X",Player[playerid][SpawnPos][0]);
		INI_WriteFloat(ufile,"Pos_Y",Player[playerid][SpawnPos][1]);
		INI_WriteFloat(ufile,"Pos_Z",Player[playerid][SpawnPos][2]);
		INI_WriteInt(ufile,"InvSize",Player[playerid][InvSize]);
		INI_WriteInt(ufile,"Adminlevel",Player[playerid][Adminlevel]);
		INI_WriteInt(ufile,"hKills",Player[playerid][Kills][0]);
		INI_WriteInt(ufile,"zKills",Player[playerid][Kills][1]);
		INI_WriteInt(ufile,"dKills",Player[playerid][Kills][2]);
		INI_WriteInt(ufile,"cItems",Player[playerid][Count][0]);
		INI_WriteInt(ufile,"cCraft",Player[playerid][Count][1]);
		INI_WriteInt(ufile,"cTrees",Player[playerid][Count][2]);

		INI_SetTag(ufile,"Inventory");

		Loop(Player[playerid][InvSize])
		{
			format(s_string,sizeof(s_string),"Model_%d",i);
			INI_WriteInt(ufile,s_string,-1);

			format(s_string,sizeof(s_string),"Amount_%d",i);
			INI_WriteInt(ufile,s_string,0);
		}
		INI_Close(ufile);
	}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		...
		case DIALOG_LOGIN:
		{
		    if(response)
		    {
		    	if(udb_hash(inputtext) == Player[playerid][Password])
		    	{
		    		INI_ParseFile(UserFile(playerid), "GetUser_%s", .bExtra = true, .extra = playerid);
		    		...
		    		ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login:", "Wrong password, try again:", "Login","");
			}
		}
		...
		case DIALOG_REGISTER:
		{
            if(response)
            {
				new INI:ufile = INI_Open(UserFile(playerid));
				INI_SetTag(ufile,"Password");
				INI_WriteInt(ufile,"Password",udb_hash(inputtext));
...
{
	format(s_string,sizeof(s_string),FILE_USER,Player[playerid][Name]);
	return s_string;
}
...
// ===================================== Y_INI =================================
public CheckUser_Password(playerid, name[], value[])
{
	Player[playerid][Password] = strval(value);
}

public GetUser_Data(playerid, name[], value[])
{
	INI_Float("Health",Player[playerid][Hunger]);
	INI_Float("Thrist",Player[playerid][Thirst]);
	INI_Float("Sleep",Player[playerid][Sleep]);
	INI_Float("Exp",Player[playerid][Experience]);
	INI_Int("Level",Player[playerid][Level]);
	INI_Float("Pos_X",Player[playerid][SpawnPos][0]);
	INI_Float("Pos_Y",Player[playerid][SpawnPos][1]);
	INI_Float("Pos_Z",Player[playerid][SpawnPos][2]);
	INI_Int("Adminlevel",Player[playerid][Adminlevel]);
	INI_Int("InvSize",Player[playerid][InvSize]);
	INI_Int("hKills",Player[playerid][Kills][0]);
	INI_Int("zKills",Player[playerid][Kills][1]);
	INI_Int("dKills",Player[playerid][Kills][2]);
	INI_Int("cItems",Player[playerid][Count][0]);
	INI_Int("cCraft",Player[playerid][Count][1]);
	INI_Int("cTrees",Player[playerid][Count][2]);

	return printf("Stats Loaded for %s (%d) !",Player[playerid][Name],playerid);
}

public  GetUser_Inventory(playerid, name[], value[])
{
	//[TO-DO] load inventory
	printf("Inventory for %d:",playerid);
...
{
	SendClientMessage(playerid,COLOR_GREEN,"Your account has been registered !");
